...
class _SlideState extends State<Slide> {
  ...
  Widget dot(bool action) {
    return new Container(
      ...
      decoration: new BoxDecoration(
        color: action ? Color.fromRGBO(1, 1, 1, 0.54) : Color.fromRGBO(1, 1, 1, 0.14),
  ...
  // 自定义的 map 函数
  List<Widget> map(List list, Function func) {
    List _list = new List<Widget>();
        ...
        children: <Widget>[
          ...
          new Container(
            ...
            child: new Row(
              ...
              children: this.map(widget.data, (item, index) {
...
this.map 是自己写的 map 函数，因为自带的不支持 index 参数。format 是一个时间格式化函数，把 `20180708` 的格式转换为类似 `今日新闻`、`07月07日` 的格式。